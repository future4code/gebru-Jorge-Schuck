...
import { CardStyled, DetailCardStyled, DetailMovieStyled, StyleTypograph } from './styled';
import { ScrollArea } from '@mantine/core';
import {Img} from "../DetailCard/styled";
...
export default function DetailCard({ detail, id }) {
    ...
    const crews = data.crew && data.crew.filter(
        ...
        return (
            <div key={crew.id}>
                ...
                <Typography variant='h2' fontSize={14}>{crew.job}</Typography>
                <Typography variant='h3' fontSize={16}>{crew.name}</Typography>
            </div>
        )
    })
    ...
    return (
        <div>
            {detail &&
                <DetailMovieStyled key={detail.id}>
                    ...
                    <StyleTypograph>
                        <Typography  fontSize={40} >{detail.title}</Typography>                  
                        <Typography  fontSize={15} display={'inline'}>{moment.utc(detail.release_date).format('DD/MM/YYYY')}</Typography>
                        <Typography  fontSize={15} display={'inline'}>{genres}</Typography>
                        <Typography  fontSize={15} display={'inline'}> {(detail.runtime / 60).toFixed()}h{detail.runtime % 60}min </Typography>
                        <Typography variant='h2' fontSize={30}>Sinopse</Typography>
                        <Typography  fontSize={22}>{detail.overview}</Typography>
                        <br></br>
        ...
        <div>
            <Typography variant='h1' fontSize={24}>Elenco original</Typography>
            <ScrollArea>
                {loading ?
